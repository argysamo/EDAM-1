    ...
    def clean_df_db_dups(self, df, tablename, dup_cols=None,
        ...
        if filter_continuous_col is not None:
            if df[filter_continuous_col].dtype == 'datetime64[ns]':
                args_contin_filter = """
                "%s" BETWEEN Convert(datetime, '%s') AND Convert(datetime, '%s')
                """ % (
                    filter_continuous_col, df[filter_continuous_col].min(),
    ...
    def __add_dataframe__(dataframe: pd.DataFrame, table='Observations', index=True,
        ...
        def load_to_database(table1, unique_columns, file_object):
            ...
            try:
                ...
                columns = ', '.join(['{}'.format(col)
                                     for col in unique_columns])
                sql = 'COPY "{}" ({}) FROM STDIN WITH CSV HEADER'.format(
                    table1, columns)
